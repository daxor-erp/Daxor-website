      ...
      <Card className="w-full max-w-[600px] shadow-none border-none bg-background">
        ...
        <CardContent className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 border rounded-md p-2 overflow-y-auto max-h-[320px]">
            <p className="mb-2 text-sm font-medium text-muted-foreground">Pick a time (IST)</p>
        ...
        <CardFooter className="flex justify-between">
          <Button variant="ghost" size="sm" onClick={() => { setDate(undefined); setTime(undefined); }}>
            Reset
          </Button>
          <Button size="sm" onClick={() => onConfirm?.({ date, time })} disabled={!date || !time}>
            {date && time ? `Confirm — ${format(date, "MMM d")} at ${time}` : "Confirm"}
          </Button>
        </CardFooter>
      </Card>
    </div>
